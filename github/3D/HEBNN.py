...
class HEBNN:
    ...
    def __init__(self, X_u, u, X_f, layers, lb, ub, a, noise):
        ...
        self.u_pred = self.net_u(self.x_u_tf, self.y_u_tf, self.z_u_tf) #观测数据
        self.f_pred = self.net_f(self.x_f_tf, self.y_f_tf, self.z_f_tf) #配置点     
    ...
    def net_u(self, x,y,z):
        u = self.neural_net(tf.concat([x,y,z],1), self.weights, self.biases)
        return u
